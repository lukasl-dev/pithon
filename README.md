...
A notebook-first IPython extension for Pi.
...
- **Notebook-first tool surface.** On session startup the extension removes
  ...
  This shrinks the tool schema and makes the model use the retained kernel. The
  behavior is configurable and enabled by default.
...
outputs can be summarized or selected before returning to the model. Kernel
output is capped per stream/result field (64 KiB by default), and the agent
should store large data in named values or files rather than print it.
...
- `/pithon` — open persistent extension settings.
...
Settings are stored atomically in `~/.pi/agent/pi-ipython.json` and apply
immediately:

- enable or disable the extension;
- disable or retain Pi's built-in file and shell tools;
- restore and snapshot the Python namespace;
- export the notebook on shutdown;
- show or hide output in collapsed tool rows; and
- select a 16, 64, or 256 KiB output limit.

For deployment-level overrides, `PI_IPYTHON_KEEP_BUILTINS=1` retains Pi's
native tools and `PI_IPYTHON_EXPORT_IPYNB=0` disables shutdown export.
